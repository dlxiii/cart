function dyef = dye(nc,dye,inttime)
...
%%
    ...
    for k=1:kbm1
       ...
       for i=1:m
           ...
           for j=1:ntsn(i)-1
               ...
               if iswetn(i1) == 0 && iswetn(i2) == 1
                   ffd=0.5*(dye(i,k)+dye(i2,k));
                   ff1=0.5*(dye(i,k)+dye(i2,k));
               elseif iswetn(i1) == 1 && iswetn(i2) == 0
                   ffd=0.5*(dye(i1,k)+dye(i,k));
                   ff1=0.5*(dye(i1,k)+dye(i,k));
               elseif iswetn(i1) == 0 && iswetn(i2) == 0
                   ffd=0.5*(dye(i,k)+dye(i,k));
                   ff1=0.5*(dye(i,k)+dye(i,k));
               else
                   ffd=0.5*(dye(i1,k)+dye(i2,k));
                   ff1=0.5*(dye(i1,k)+dye(i2,k));
               end
       ...
       for i=1:ncv_i
           ...
           fij2=dye(ib,k) + dltxncve(i,1)*pdyepx(ib) + dltyncve(i,1)*pdyepy(ib);

           s1min=min(dye(nbsn(ia,1:ntsn(ia)-1),k));
           s1min=min(s1min, dye(ia,k));
           s1max=max(dye(nbsn(ia,1:ntsn(ia)-1),k));
           s1max=max(s1max, dye(ia,k));
           s2min=min(dye(nbsn(ib,1:ntsn(ib)-1),k));
           s2min=min(s2min, dye(ib,k));
           s2max=max(dye(nbsn(ib,1:ntsn(ib)-1),k));
    ...
    for i=1:m
       if iswetn(i)*iswetnt(i) == 1
           for k=1:kbm1
               if k == 1
                   temp=-wts(i,k+1)*(dye(i,k)*dz(i,k+1)...
                       +dye(i,k+1)*dz(i,k))/(dz(i,k)+dz(i,k+1));
               elseif k == kbm1
                       temp= wts(i,k)*(dye(i,k)*dz(i,k-1)...
                           +dye(i,k-1)*dz(i,k))/(dz(i,k)+dz(i,k-1));
               else
    ...
    for i=1:m
     if iswetn(i)*iswetnt(i) == 1
         for k=1:kbm1
             ...
             dyef(i,k)=(dye(i,k)-xflux(i,k)/art1(i)*(dti/(dt(i)*dz(i,k))))...
                 *(dt(i)/dtfa(i));
         end
     else
         for k=1:kbm1
    ...
    for i = 1:m
        if iswetn(i) == 1
            ...
            vhpf(i,1) = -dti *(wfsurf(i)-swradf(i)+...
                rad(i,1)-rad(i,2)) / (-dz(i,1)*d(i)) - dyef(i,1);
    ...
    for k = 2:kbm2
        for i = 1:m
            if iswetn(i) == 1
                ...
                vhpf(i,k) = (cf(i,k)*vhpf(i,k-1)-dyef(i,k)+...
